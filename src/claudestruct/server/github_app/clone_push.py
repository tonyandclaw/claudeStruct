"""Git clone + push + PR-open helpers (W6.6 worker-side integration).

The worker calls :func:`open_pr_as_bot` after a successful run to:

1. Mint an installation token via the cached
   :class:`InstallationTokenCache`.
2. Clone the repo with the install token via git-over-https.
3. Create a branch off the PR's base branch and push the run's
   working-tree changes as a commit.
4. Open a PR as the App (via the already-shipped
   :func:`create_pull_request` primitive).

Git-over-https is used (not the Contents API) because multi-file /
rename / binary diffs are handled correctly by native git rather
than requiring callers to hand-encode every file as base64.

Lives in its own module so the transport-heavy clone/push concern
doesn't bloat ``github_app/__init__.py``. The PR-open helpers
in this file import a handful of names from the parent package
(``mint_app_jwt`` / ``get_ref_sha`` / ``create_pull_request`` /
``InstallationTokenCache`` / ``GitHubAppConfig``) — the parent
defines all of those above the bottom-of-file re-export, so the
...
log = logging.getLogger("claudestruct.github_app.clone_push")
...
def _prepare_repo_push(
    *,
    ...
    """
    # Late import: avoids a top-of-file circular reference to the
    # still-loading parent package during module initialisation.
...
def open_pr_as_bot(
    *,
    ...
    Best-effort: if the push fails after the PR is opened we can't
    roll back the PR creation, so we let the operator know via the
    verdict comment that the branch may be stale.

    ``cfg`` and ``cache`` are typed as ``Any`` here to avoid the
    cycle on import — both are defined in the parent package and
    only used pass-through.
    """
    # Late imports for the same circular-resolution reason as
    # _prepare_repo_push.
