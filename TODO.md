...
- [~] **W8.7 — Status page + SLO dashboard** (fleet-wide + per-tenant SLO + API latency middleware shipped; external status page deferred)
  ...
  - **SLO error-budget burn-rate alerts (this PR — A.2)**: `compute_burn_rate_alerts(session)` in `alerts.py` runs the Google SRE multi-window detector against `Run.status` per org. Two windows: 1 h (fast burn, ≥ 14.4× target → critical / page) and 6 h (slow burn, ≥ 6× target → warning / ticket). `BURN_RATE_MIN_RUNS=10` floor avoids the "single failure dominates the rate" footgun at low traffic. New `Alert(kind="slo_error_budget_burn")` shape carries window + total_runs + failed + error_rate + burn_rate + threshold. Wired into `run_alert_pass` so the existing `cs serve alerts [--watch]` daemon now dispatches three detector kinds (cost-regression, team-budget burn, SLO budget burn) through one notifier surface. 7 new tests in `tests/test_alerts.py` covering the low-traffic skip, below-threshold no-op, fast-burn critical (50× over 1 h), slow-burn warning (10× over 6 h), sort order, summary content, end-to-end notifier dispatch.
