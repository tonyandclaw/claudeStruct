...
- [x] **W6.2 — HTTP REST API** (session cookie hardening shipped)
  ...
  - **Per-language SDK stubs (this PR — B.6)**: `sdks/python/` (`claudestruct-sdk`, version `0.0.1a0`) + `sdks/typescript/` (`@claudestruct/sdk`, version `0.0.1-alpha`). Each ships a `Client` entry point with `.runs / .dashboard / .keys / .budget` resource namespaces, plain dataclasses / TS interfaces mirroring `server/schema.py`, and a typed exception hierarchy (`AuthError` 401 / `BudgetExceededError` 402 / `ForbiddenError` 403 / `NotFoundError` 404 / `ServerError` 5xx). Both SDKs read `X-CS-Api-Version` on every response and warn when it drifts from what the SDK targets. Single transport per side (Python: `httpx.Client` / TypeScript: native `fetch`); no retries by design — the application layer owns that. Pre-stable: shape may shift before 0.1.0. 13 Python smoke tests + 12 TypeScript smoke tests lock the public surface (constructor invariants, status-code mapping, auth header, version-drift warning, resource method URLs).
...
- [x] **W6.5 — Shared dashboard** ✅ (multi-user view + cost-regression alerts + budget-cap team rollups all shipped)
  ...
  - **Team budget-cap burn alert (this PR)**: `compute_team_budget_alerts(session)` in `alerts.py` rolls up per-team spend in the current Stripe billing window (UTC calendar month fallback) and emits a `TeamBudgetFinding` for any team that has burned ≥ 80 % of the org's `TIER_USD_CAPS`. Dispatched as `Alert(kind="team_budget_burn")` — warning at ≥ 80 %, critical at ≥ 100 %. Uncapped tiers (team / business) skipped (no denominator); empty teams skipped (no spend by definition). Findings sort by ratio descending so operators see the most-burned team first. Wired into `run_alert_pass` so the existing `cs serve alerts [--watch]` daemon picks it up alongside cost-regression alerts. 8 new tests in `tests/test_alerts.py` (uncapped, below threshold, warning, critical, multi-member sum, empty team, sort order, end-to-end dispatch).
...
- [~] **W7.3 — Skills marketplace** (claw-squad; registry hosting deferred)
  ...
  - **Cosign-style signature verification (this PR)**: optional `signature: {algorithm, publicKeyPem, signatureBase64}` field on `SkillManifest` — supports `ed25519` (preferred — small keys, fast verify) and `rsa-pss-sha256` (broader compatibility). New `verifySkillSignature(body, sig)` uses Node stdlib `crypto.verify` (no cosign binary or Sigstore/Rekor network round-trip needed for a v1 — that layer can land later). `installSkill` runs signature verification BEFORE the sha256 check so a wrong key fails fast with a key-drift error message. New env `CLAW_SKILLS_REQUIRE_SIGNATURE=1` rejects unsigned manifests at install — the lever air-gapped / regulated environments flip when sha256 alone isn't sufficient. 18 new tests in `claw-squad/tests/skills-registry.test.ts`: ed25519 happy path / tampered body / wrong signer / malformed PEM / empty signature / RSA-PSS, parseManifest signature shape (4 cases — no-sig back-compat, valid block, unknown algorithm, malformed PEM), installSkill (4 cases — happy, wrong sig, REQUIRE_SIGNATURE=1 reject, REQUIRE_SIGNATURE unset still permits).
  - Pending: actual hosting at `skills.claudestruct.dev` (manual infra step)
...
- [x] **W8.4 — Append-only hash-chained audit log**
  ...
  - **Customer-success instrumentation (this PR — A.5)**: OAuth callback now emits `auth.login.success` and `POST /v1/auth/logout` emits `auth.logout` (idempotent — second logout for an already-revoked session does NOT double-audit). Payload records `provider` + the `UserSession` row id; the bearer cookie value itself is never written to the audit chain. `docs/admin-events.md` ships the full event catalogue + PII rules so support knows what's safe to read.
  ...
  - Tests: `tests/test_audit.py` (17 cases) + `tests/test_session_cookies.py` (3 new — auth.logout audit, idempotent logout no-double-audit, auth.login.success on github callback).
...
- [x] **W8.6 — Customer-managed encryption keys (CMEK)**
  ...
  - GCP KMS provider still `NotImplementedError` — google-cloud-kms uses a different SDK + auth model (workload identity / SA JSON); follow-up alongside a future GCP-side secrets adapter.
...
- [~] **W8.7 — Status page + SLO dashboard** (fleet-wide + per-tenant SLO + API latency middleware shipped; external status page deferred)
  ...
  - **API request-latency middleware**: `RequestLatencyMiddleware` in `latency.py` times every HTTP request via `time.perf_counter()` and records per-route p50/p95/p99 in a thread-safe `LatencyTracker`. Route resolved via `getattr(route_obj, "path")` to avoid FastAPI's unhashable Route object. `GET /v1/slo/latency` (unauthenticated) returns Prometheus text exposition format.
  - **Webhook error counter (this PR — C.3)**: `server/webhook_metrics.py` ships a thread-safe `WebhookErrorCounter` keyed by `(path, reason)`. The 4 swallow sites in `worker.py` (verdict comment / check-run open / check-run complete / PR-open) plus the Stripe subscription fetch in `routers/billing.py` plus the GitHub ack-comment path in `routers/github.py` now bump the counter before logging, so an alert can fire on `rate(claudestruct_webhook_errors_total{path="github.pr_open"}[5m])` without grep-parsing log lines. Reason labels distinguish `github_app_error` (upstream returned an error our wrapper recognises) from `unexpected` (we crashed). Exposed via new unauthenticated `GET /v1/slo/webhook_errors` (Prometheus text). 11 new tests in `tests/test_webhook_metrics.py`: counter math, label escaping, concurrent-bump invariance, unauth route shape, end-to-end worker swallow paths bump the right `(path, reason)`.
  ...
  - **SLO error-budget burn-rate alerts (A.2)**: `compute_burn_rate_alerts(session)` in `alerts.py` runs the Google SRE multi-window detector against `Run.status` per org. Two windows: 1 h (fast burn, ≥ 14.4× target → critical / page) and 6 h (slow burn, ≥ 6× target → warning / ticket). `BURN_RATE_MIN_RUNS=10` floor avoids the "single failure dominates the rate" footgun at low traffic. New `Alert(kind="slo_error_budget_burn")` wired into `run_alert_pass`.
  - **Public status endpoint (this PR — A.6)**: new `GET /status` (unauthenticated) aggregates DB connectivity (`SELECT 1` round-trip), worker queue depth (queue_depth ≥ 50 → degraded, ≥ 500 → down), and fleet-wide 1 h error rate (≥ 0.5% → degraded, ≥ 5% → down). `overall = worst(components)`. Payload is exclusively global aggregates — `test_status_endpoint_no_tenant_data_leaks` asserts no org slug / user email / run id appears in the body. `recent_error_rate_1h` returns null below the 10-run floor so the status page doesn't flap on a coincidence. `docs/status.md` documents the response shape, thresholds, anti-flap rules, and the "why not authenticated" trade-off. 9 new tests in `tests/test_status_endpoint.py`.
  - Pending: external `status.claudestruct.dev` SPA / public incident timeline (manual infra — the endpoint is the API; the SPA is the deferred work)
