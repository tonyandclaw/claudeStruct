...
- [x] **W6.5 — Shared dashboard** ✅ (multi-user view + cost-regression alerts + budget-cap team rollups all shipped)
  ...
  - **CLI**: `cs serve alerts [--sigma 2.0] [--lookback-days 30] [--check-recent-hours 24] [--watch] [--interval 900]` runs the detector once (default) or as a long-lived scheduler; `--watch` polls every `--interval` seconds until SIGINT. Survives transient pass errors (DB blip, etc.) — logs + continues so a one-off failure doesn't tear the daemon down. `--max-iterations` is a test seam.
  - 25 prior + 14 new tests = 39 covering: stddev math (empty, single, population formula); detector behaviour (insufficient baseline skip, zero-stddev skip, below-mean not flagged, above-threshold flagged, old-spike-not-re-alerted, failed-runs excluded, per-org isolation, sort order); severity ladder (info/warning/critical/inf); dispatch round-trip via `_CapturingNotifier`; LogNotifier WARNING vs ERROR levels; SlackWebhookNotifier (empty URL rejected, payload shape, non-200 swallowed); `default_notifier` factory env-driven selection (log/slack/email + missing-host/missing-recipients/invalid-port for email); `EmailNotifier` (rejects empty host/sender/recipients, send w/ STARTTLS+login, no-credentials skip-login, swallows send/login/STARTTLS/connect failures, mixed-separator parsing); `serve alerts --watch` (bounded passes, --interval=0 rejected, transient-failure recovery)
  - **EmailNotifier (this PR)**: `EmailNotifier` provider in `notify.py` sends one stdlib `email.message.EmailMessage` per alert via `smtplib.SMTP`. Config via env (`CLAUDESTRUCT_SMTP_HOST` / `_PORT` / `_USER` / `_PASSWORD` / `_STARTTLS` / `_FROM` / `_TO`). Comma-or-semicolon-separated recipient list parsed via `_split_recipients`. Connection / STARTTLS / login / send failures are caught and logged at WARNING — matches `SlackWebhookNotifier`'s swallow-and-log policy so the cron driver never dies on a transient SMTP blip. `default_notifier()` picks it up when `CLAUDESTRUCT_NOTIFY_PROVIDER=email`.
  - **Long-running scheduler (this PR)**: `cs serve alerts --watch --interval N` polls the detector every N seconds (default 900s = 15min) instead of one-shot exiting. SIGINT prints `alerts: stopping watcher` and exits 0 cleanly — no traceback in journalctl. A pass-level exception logs `alerts[N]: error: …` to stderr and the loop continues to the next interval (transient DB / network failures are not fatal).
...
- 2026-05-07 — W6.5 close-out advance: EmailNotifier + scheduler-mode `cs serve alerts --watch` ready for PR push:
  - **EmailNotifier**: third provider in `src/claudestruct/server/notify.py` (after `LogNotifier` + `SlackWebhookNotifier`). Stdlib-only (`smtplib` + `email.message.EmailMessage`); config via `CLAUDESTRUCT_SMTP_HOST/_PORT/_USER/_PASSWORD/_STARTTLS/_FROM/_TO`. Connection / STARTTLS / login / send failures swallowed-and-logged so cron driver survives transient SMTP blips. `default_notifier()` selects when `CLAUDESTRUCT_NOTIFY_PROVIDER=email`.
  - **`cs serve alerts --watch`**: same detector / dispatcher as the one-shot path, but loops every `--interval` seconds (default 900s = 15min) until SIGINT. Per-pass exceptions log + continue (transient DB blip ≠ daemon death). `--max-iterations` is a bounded-loop test seam; `--interval 0` rejected with a clear error before any work begins. SIGINT prints a clean `alerts: stopping watcher` line — no traceback in `journalctl`.
  - 14 new tests covering: 9 EmailNotifier cases (host/sender/recipient validation, STARTTLS+login send, no-credentials skip-login, swallow connect/STARTTLS/login/send failures); 4 factory cases (`email` provider, missing host, missing recipients, invalid port); 1 `_split_recipients` mixed-separator case; 3 watch-mode CLI cases (bounded passes + sleep injection, `--interval 0` rejected, transient-failure recovery)
  - 624 Python tests pass (was 610); ruff clean
  - Pending under W6.5: per-team budget-cap rollup alert kind (small follow-up; same notifier surface)
