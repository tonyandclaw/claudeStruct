...
- [x] **W6.5 — Shared dashboard** ✅ (multi-user view + cost-regression alerts + budget-cap team rollups all shipped)
  ...
  - **Email notifier (this PR)**: third concrete `Notifier` impl alongside `LogNotifier` and `SlackWebhookNotifier`. Stdlib `smtplib` (no extra dep), STARTTLS-required (we deliberately don't fall back to plaintext — alert bodies routinely name orgs and quote details). Env-driven config via `CLAUDESTRUCT_NOTIFY_PROVIDER=email` + `CLAUDESTRUCT_EMAIL_SMTP_HOST/PORT/USERNAME/PASSWORD`, `_FROM`, `_TO` (comma-separated). `smtp_factory` injection seam keeps the test suite offline. 10 new tests in `tests/test_alerts.py`: ValueError on missing host/from/to, happy-path sendmail call shape, login skipped when no username, STARTTLS-unsupported drops alert, sendmail failure swallowed, default_notifier env dispatch (happy + missing-env + invalid-port).
  - Pending: budget-cap rollups per team (a "team has burned 80% of monthly cap" alert kind that uses the same notifier surface — small follow-up); long-running scheduler (currently `--once`-only)
